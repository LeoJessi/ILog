 ...
 * <p>
 * A {@link Logger} is always generated and mostly accessed by {@link ILog}, but for customization
 * purpose, you can configure a {@link Logger} via the {@link Builder} which is returned by
 * {@link ILog} when you trying to start a customization using {@link ILog#tag(String)}
 ...
 * the {@link Logger} assuming that you are using the {@link ILog} directly.
 */
public class Logger {
    ...
    private void printlnInternal(int logLevel, String msg) {
         ...
         * 如果需要显示调用的位置  则打印出信息
         * */
        if (logConfiguration.bePrintPosition) {
            StackTraceElement element = StackTraceUtil.getCroppedRealStackTrack(new Throwable().getStackTrace(),
                    logConfiguration.stackTraceOrigin,
                    logConfiguration.stackTraceDepth)[0];
            position = "<===============  " + element.getFileName() + " --- Line: " + element.getLineNumber()
                    + " --- Method: " + element.getMethodName() + "  ===============>";
        }

        msg = position + "\n" + (logConfiguration.withBorder
